...
class MainSuite extends FunSuite {
  ...
  test("createCatalog should create products with correct BigDecimal precision") {
    val catalog = createCatalog()
  ...
  test("shopping basket logic should calculate correct subtotal for single product") {
    val catalog = createCatalog()
    val apple = catalog.getByName("Apples").get
  ...
  test("shopping basket logic should calculate correct subtotal for multiple products") {
    val catalog = createCatalog()
    val apple = catalog.getByName("Apples").get
    val soup = catalog.getByName("Soup").get
    val bread = catalog.getByName("Bread").get
  ...
  test("shopping basket logic should handle mixed quantities") {
    ...
    assertEquals(basket.subtotal, BigDecimal(2.00 + 1.95)) // 2*1.00 + 3*0.65 = 2.00 + 1.95 = 3.95
