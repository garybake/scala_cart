...
class OfferServiceSuite extends FunSuite {
  ...
  test("OfferService.inMemory should handle offers with no applicable products") {
    val offers = List(
      PercentageOffSingleProduct(
        product = apple,
        percent = BigDecimal("10"),
  ...
  test("OfferService.default should create service with predefined offers") {
    ...
    val soupBreadOffer = service.availableOffers.find(_.label == "Buy 2 Soup, get Bread 50% off")
  ...
  test("OfferService.default should have correct apple offer") {
    val service = OfferService.default
    val appleOffer = service.availableOffers.find(_.label == "Apples 10% off").get
  ...
  test("OfferService.default should have correct soup-bread offer") {
    val service = OfferService.default
    val soupBreadOffer = service.availableOffers.find(_.label == "Buy 2 Soup, get Bread 50% off").get
    
    soupBreadOffer match {
      case BuyNGetMOfOtherAtPercent(triggerProduct, triggerQty, targetProduct, targetQtyPerTrigger, targetPercentOff, _, active) =>
