...
class OfferSuite extends FunSuite {
  ...
  test("PercentageOffSingleProduct should apply correct discount for single item") {
  ...
  test("PercentageOffSingleProduct should apply correct discount for multiple items") {
    val offer = PercentageOffSingleProduct(
      product = apple,
  ...
  test("PercentageOffSingleProduct should return discount of 0 when product not in basket") {
    val offer = PercentageOffSingleProduct(
      product = apple,
      percent = BigDecimal("10"),
      label = "Apples 10% off",
  ...
  test("BuyNGetMOfOtherAtPercent should apply discount when trigger quantity met") {
  ...
  test("BuyNGetMOfOtherAtPercent should apply discount for multiple trigger quantities") {
    val offer = BuyNGetMOfOtherAtPercent(
      triggerProduct = soup,
      triggerQty = 2,
      targetProduct = bread,
  ...
  test("BuyNGetMOfOtherAtPercent should not apply discount when trigger quantity not met") {
    val offer = BuyNGetMOfOtherAtPercent(
      triggerProduct = soup,
      triggerQty = 2,
      targetProduct = bread,
  ...
  test("BuyNGetMOfOtherAtPercent should limit discount to available target quantity") {
    val offer = BuyNGetMOfOtherAtPercent(
      triggerProduct = soup,
      triggerQty = 2,
      targetProduct = bread,
    ...
    val basket = Basket(Map(soup -> 6, bread -> 1)) // Could get 3 bread discounts, but only 1 bread available
    val discounts = offer.applyTo(basket)
    
    assert(discounts.length == 1)
    assert(discounts.head.amount == BigDecimal("0.60")) // 50% of 1.20 (only 1 bread)
  }
  ...
  test("BuyNGetMOfOtherAtPercent should return empty list when no trigger product") {
    val offer = BuyNGetMOfOtherAtPercent(
      triggerProduct = soup,
      triggerQty = 2,
      targetProduct = bread,
  ...
  test("BuyNGetMOfOtherAtPercent should return empty list when no target product") {
    val offer = BuyNGetMOfOtherAtPercent(
      triggerProduct = soup,
      triggerQty = 2,
      targetProduct = bread,
  ...
  test("BuyNGetMOfOtherAtPercent should handle decimal percentages") {
    ...
    val expectedAmount = BigDecimal("1.20") * BigDecimal("33.33") / BigDecimal("100")
  ...
  test("Offer should respect active flag") {
    ...
    assert(inactiveDiscounts.nonEmpty) // Note: active flag is not currently used in applyTo logic
  }
  
  test("BuyNGetMOfOtherAtPercent should handle edge case with trigger quantity of 1") {
    val offer = BuyNGetMOfOtherAtPercent(
      triggerProduct = soup,
      triggerQty = 1,
      targetProduct = bread,
