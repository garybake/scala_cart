 ...
 * 
 * A Product defines the essential attributes of an item that customers can purchase,
 * including its unique identifier (SKU), display name, unit of measurement, and price.
 * Products are immutable and serve as the foundation for basket operations and
 * discount calculations.
 * 
 * @param sku The Stock Keeping Unit - a unique identifier for the product
 * @param name The human-readable name of the product for display purposes
 * @param unit The unit of measurement (e.g., "tin", "loaf", "bottle", "bag")
 * @param unitPrice The price per unit in the system's currency
 * 
 * @example
 * {{{
 ...
 * }}}
 * 
 * @note Products are case classes, so they provide automatic equality comparison,
